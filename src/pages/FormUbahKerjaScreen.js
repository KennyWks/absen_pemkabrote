...
export default function FormUbahKerjaScreen({route, navigation}) {
  ...
  useEffect(() => {
    setJudul({value: judul_kegiatan, error: ''});
    setDeskripsi({value: deskripsi_kegiatan, error: ''});
    setMulai({value: mulai_kegiatan, error: ''});
    setSelesai({value: selesai_kegiatan, error: ''});
  }, []);
  ...
  const submitKegiatan = async () => {
    ...
    try {
      ...
      const response = await patchData(`/pekerjaan/${pekerjaan_id}`, {
